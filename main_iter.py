...
if __name__ == '__main__':
    ...
    cv2.imshow("result_global",result)
    ...
    # todo do iteration util the alignment get best
    for iter in range(5):
        ...
        mesh_boxes_src, src_x_num, src_dst_num = get_mesh_boxes(src_warp)
        mesh_boxes_dst, dst_x_num, dst_y_num = get_mesh_boxes(dst_warp)
        weight_lines, location_lines = get_line_point(src_warp_gray, mesh_boxes_src, src_warp=None)
        ...
        result2_gray = cv2.cvtColor(bg,cv2.COLOR_BGR2GRAY)
        mask = result2_gray*bg_gray

        src_warp = final_result
        dst_warp = bg

        cv2.imshow("result"+str(iter),result2)
